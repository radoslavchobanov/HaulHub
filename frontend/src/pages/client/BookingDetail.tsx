    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <div className="card">
            <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-3">Your Hauler</h3>
            <Link to={`/haulers/${booking.hauler.id}`} className="flex items-center gap-3 hover:opacity-80 transition-opacity">
              ...
              <div>
                <p className="font-medium text-gray-900 dark:text-gray-100">{booking.hauler.full_name}</p>
                {haulerProfile && (
                  <div className="flex items-center gap-1 mt-0.5">
                    <StarRating rating={parseFloat(haulerProfile.rating_avg)} size="sm" />
                    <span className="text-xs text-gray-500 dark:text-gray-400">({haulerProfile.review_count})</span>
                  </div>
                )}
              </div>
            </Link>
          ...
          {booking.status === 'completed' && booking.can_review && (
            <div className="card">
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">How was your experience with this Hauler?</p>
          ...
          {booking.status === 'completed' && !booking.can_review && (
            <div className="card bg-green-50 border-green-200 dark:bg-green-900/20 dark:border-green-800">
              <p className="text-sm text-green-800 dark:text-green-400 text-center">✓ Job completed</p>
            </div>
          )}
        </div>
      </div>
